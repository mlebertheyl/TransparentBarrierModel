...
sf4perimeter <- st_as_sf(sp4perimeter)
pl.sel.sf <- st_as_sf(pl.sel)
poly.water.book.sf <- st_difference(pl.sel.sf, sf4perimeter)
poly.water.book <- as(poly.water.book.sf, "Spatial")
...
plot(pl.sel.sf, col = alpha("skyblue", 0.5), asp = 1)
plot(sf4perimeter, add = TRUE, col = alpha(gray(0.9), 0.5))
plot(poly.water.book.sf, col = alpha("skyblue", 0.5))

plot(pl.sel.sf, col = alpha("skyblue", 0.5), asp = 1)
plot(sf4msh, add = TRUE, col = alpha(gray(0.9), 0.5))
...
################################################################################
#until here is ArticleCode Dng Example or DugongExample
################################################################################
#from here is tbm trial following Elias' tutorial, it's just using fm_mesh_2d

sea_list.sf <- lapply(idx.spb1, function(n) st_polygon(list(sp.b1_@polygons[[1]]@Polygons[[n]]@coords))) 
length(sea_list) == length(sea_list.sf)

bathym.sf <- st_as_sf(bathym_poly)
bathym.sf <- st_transform(bathym.sf, crs_km)

sf.b1.ex <- st_as_sf(sp.b1.ex)
sf::st_crs(sf.b1.ex) <- sf::st_crs(bathym.sf)

p1 <- ggplot() + geom_sf(data = bathym.sf) + 
  theme_minimal()

p2 <- ggplot() + geom_sf(data = sf4msh) + 
  theme_minimal()
...
#(p1 | p2) / p3     # custom layout
#so sf4msh has the holes and bathym.sf has the entire bathymetry without the holes

#mesh with bathym.sf
#mesh.sf <- fm_mesh_2d(loc.domain = bathym.sf, max.edge = c(1,5)*max.edge.length, offset = c(max.edge.length, bound.outer2), cutoff   = 0.05)
#doesn't work
...
###########################################################################################################
#in inlabru i shouldn't need to do w right?
#This is not working for me, some problem with the geometry
...
w <- sapply(1:length(dmesh), function(i) {
  if (length(st_intersects(dmesh_sf[i, ], domainsf)[[1]]) == 1)
    return(st_area(st_intersection(dmesh_sf[i, ], domainsf)))
  else {
    return(0)
  }
})
...
lmat <- inla.spde.make.A(mesh.dng, xy.sf)
