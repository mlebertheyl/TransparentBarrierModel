...
fig.cap = Coarse mesh example with barriers in red. On the left the outer polygon shows the extended mesh, and on the right the region is zoomed in so it only shows the area of interest.
Row one shows the configuration with two barriers and a canal, and row two the configuration with one barrier dividing the study area.


```{r, echo=FALSE, out.width="50%", fig.cap= "Delaunay triangulation for the different geometries used"}
...
img <- image_append(c(a,b), stack = TRUE)
#image_write(img, "plots/overleaf/mesh.png")
```
...
prior range, prior.range = c(6, 0.5)
prior sigma c(1,0.1)

```{r, eval=FALSE, echo=FALSE, out.width="50%", fig.cap= "Correlation plots"}
###0.01
r <- magick::image_read("plots/corr2points_m1point1/0.01.png")
r <- magick::image_crop(r, geometry = "480x480-90+000")
...
i <- magick::image_crop(i, geometry = "480x480-00+000")

a <- image_append(c(r,t,y,u,i), stack = F)
...
s <- image_append(c(r,t,y,u,i), stack = F)
...
d <- image_append(c(r,t,y,u,i), stack = F)
...
f <- image_append(c(r,t,y,u,i), stack = F)
...
g <- image_append(c(r,t,y,u,i), stack = F)
...
h <- image_append(c(r,t,y,u,i), stack = F)
...
j <- image_append(c(r,t,y,u,i), stack = F)

###

image_append(c(a,s,d,f,g,h,j), stack = TRUE)
```

```{r, eval=FALSE, echo=FALSE, out.width="50%", fig.cap= "Delaunay triangulation for the different geometries used"}
...
```{r, echo=FALSE, out.width="100%", fig.cap= "Correlation plots range fraction 1, 0.2, 0.3, 0.5, 0.7, 0.8, 1"}
...
img <- image_append(c(aa,dd), stack = F)

#image_write(img, path = "plots/overleaf/corr2points.png", format = "png")
```
...
```{r, echo=FALSE, out.width="70%", fig.cap= "Delaunay triangulation for the different geometries used"}
#r <- magick::image_read("plots/mesh/geom1.2.mesh.png")
#r <- magick::image_scale(r, "190")
a <- magick::image_read("plots/geom1.2_2/all.png")
#a <- image_append(c(r,t), stack = F)

#r <- magick::image_read("plots/mesh/geom1.2.mesh.png")
#r <- magick::image_scale(r, "190")
s <- magick::image_read("plots/geom2/all.png")
...
```{r, echo=FALSE}
r <- magick::image_read("plots/satelite.png")
r <- image_scale(r, "390")
...
a <- image_append(c(r), stack = F)
s <- image_append(c(q,w,e), stack = F)
